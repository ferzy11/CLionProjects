...
int num;
    ...
    while (num != 0)                                                                                // 0 is not a perfect number so it is used as a sentinel
    {
        int div = 2, sum = 0;
        cout << "\n\n[Enter a number]\n[Press 0 to exit]\n\n";
        cin >> num;

        if (num !=0)                                                                                // use an if to prevent the code from running everything before exiting if a 0 is input
        ...
        cout << "1";
                ...
                for (div; div < num; div++) // divisor (div) start at 2 since 1 is not a perfect number, if divisor is less than num, then go in loop. After, n increment)
                    ...
                    if (num == 1)                                                                   // 1 is not a perfect number, ask the user to input a number higher than 1
                        {
                            cout << " is not a perfect number, please enter a number higher than 1" << endl;

                        }
                        else if (sum == num)                                                        // if the sum of all divisors is == to the number input, then it is a perfect number
